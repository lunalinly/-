(() => {
  ...
  function normalizeData() {
    ...
    data.flows.forEach(flow => (flow.routes || []).forEach(route => {
      ...
      const source = data.fields.find(fieldItem => fieldItem.code === route.sourceCode);
      if (source) {
        source.fillRules ||= [];
        const migrated = { values: clone(route.values || [route.value || ""]), assignments: clone(route.assignments) };
        if (!source.fillRules.some(rule => JSON.stringify(rule) === JSON.stringify(migrated))) source.fillRules.push(migrated);
      }
      route.assignments = [];
    }));
    ...
    data.questions.forEach(question => {
      if (String(question.answerText || "").trim()) return;
      const legacy = data.templates.find(template => template.q === question.id && isSharedBranch(template.branch));
      if (legacy?.text) question.answerText = legacy.text;
    });
    data.flows.forEach(flow => {
      if (flow.question === "共用" || !Array.isArray(flow.answerParts)) return;
      flow.answerParts = flow.answerParts.filter(part => !(part.question === flow.question && isSharedBranch(part.branch)));
    });
  ...
  function qidForName(name) { return name === "共用" ? "GLOBAL" : (data.questions.find(q => q.name === name)?.id || ""); }
  function qnameForId(id) { return id === "GLOBAL" ? "共用" : (data.questions.find(q => q.id === id)?.name || "未選題目"); }
  ...
  function renderBranchReuseBar(show) {
    ...
    const unique = [...new Map(data.flows.filter(flow => !isSharedBranch(flow.branch) && (!currentQuestion || flow.question === currentQuestion || flow.question === "共用")).map((flow, index) => [`${flow.question}|${flow.branch}`, { flow, index: data.flows.indexOf(flow) }])).values()];
  ...
  function useExistingBranch() {
    ...
    data.flows.push({ question: source.question, branch: source.branch, steps: [], routes: clone(source.routes || []), answerParts: clone(source.answerParts || [{ question: source.question, branch: source.branch }]), next: source.next || "" });
  ...
  function answerAnchorChoices(value) {
    const groups = new Map();
    [...new Map(data.flows.filter(flow => !isSharedBranch(flow.branch)).map(flow => [`${flow.question}|${flow.branch}`, flow])).values()]
      .sort((a, b) => String(a.question).localeCompare(String(b.question), "zh-Hant") || String(a.branch).localeCompare(String(b.branch), "zh-Hant"))
      .forEach(flow => {
        if (!groups.has(flow.question)) groups.set(flow.question, []);
        groups.get(flow.question).push(flow);
      });
    return `<option value="">請選擇答案分支…</option>` + [...groups].map(([question, flows]) =>
      `<optgroup label="${esc(question)}">${flows.map(flow => {
        const key = JSON.stringify({ question: flow.question, branch: flow.branch });
        return `<option value="${esc(key)}" ${key === value ? "selected" : ""}>${esc(flow.branch)}</option>`;
      }).join("")}</optgroup>`
    ).join("");
  }
  ...
  function fieldForm(item) {
    const inputKind = item.multiline ? "textarea" : (item.type === "date" ? "date" : "text");
    const typeOptions = `<option value="text" ${inputKind === "text" ? "selected" : ""}>單行文字</option><option value="textarea" ${inputKind === "textarea" ? "selected" : ""}>多行文字</option><option value="date" ${inputKind === "date" ? "selected" : ""}>日期</option>`;
  ...
  function answerBranchChoices(flow, selectedPart) {
    const unique = [...new Map(data.flows.filter(item => !isSharedBranch(item.branch) && (item.question === flow.question || item.question === "共用")).map(item => [`${item.question}|${item.branch}`, { question: item.question, branch: item.branch }])).values()];
    const selected = answerPartValue(selectedPart);
    return unique.map(part => {
      const value = answerPartValue(part);
      return `<option value="${esc(value)}" ${value === selected ? "selected" : ""}>${esc(part.question)} · ${esc(part.branch)}</option>`;
    }).join("");
  }
  ...
  function inheritedVariablesFor(flow) {
    ...
    previousParts.forEach(part => {
      const q = qidForName(part.question);
      data.variables.filter(variable =>
        variable.q === q && (variable.branch === part.branch || isSharedBranch(variable.branch))
      ).forEach(variable => {
        if (!ownCodes.has(variable.code) && !inherited.has(variable.code)) {
          inherited.set(variable.code, { variable, source: `${part.question} · ${part.branch}` });
        }
      });
    });
  ...
  function inheritedVariableCard(entry) {
    const variable = entry.variable;
    const kind = variable.multiline ? "多行文字" : (variable.type === "date" ? "日期" : "單行文字");
    return `<div class="subrecord-card inherited-variable-card"><div class="subrecord-head"><div><b>${esc(variable.label)}</b><small>從前面分支自動帶入：${esc(entry.source)}</small></div><button type="button" data-edit-field-code="${esc(variable.code)}">編輯欄位</button></div><div class="linked-branch"><span>${esc(variable.category || "未分類")}　${esc(kind)}${variable.required ? "　必填" : ""}</span></div></div>`;
  }
  ...
  function branchForm(flow) {
    ...
    return `<section class="editor-section wide"><div class="editor-section-title"><div><b>分支基本資料</b><small>相同中文名稱可以重複使用。</small></div></div><div class="editor-grid">${field("所屬題目", "question", flow.question, { type: "select", choices: questionChoices(flow.question), required: true })}<label class="studio-field"><span>中文分支名稱 ＊</span><input name="branch" list="branchSuggestions" value="${esc(flow.branch)}" required><datalist id="branchSuggestions">${branchSuggestions()}</datalist><small>可選既有名稱，或直接輸入新的中文名稱。</small></label>${field("走完後的下一步", "next", flow.next, { wide: true, type: "textarea", rows: 3 })}</div></section>
      ...
      <section class="editor-section wide"><div class="editor-section-title"><div><b>需要填入的欄位</b><small>先顯示前面答案分支自動帶入的欄位，再顯示目前分支自己的欄位；相同欄位只顯示一次。</small></div></div>${inheritedRows ? `<div class="route-assignment-head"><b>前面分支自動帶入（${inheritedVariables.length}）</b></div><div class="branch-variable-list">${inheritedRows}</div>` : ""}<div class="route-assignment-head"><b>目前分支自己的欄位</b></div>${reuseControls}<div class="branch-variable-list">${variableRows}</div></section>
      ...
      <section class="editor-section wide"><div class="editor-section-title"><div><b>此分支的獨立答案文字</b><small>只編輯「${esc(flow.branch)}」本身的文字；組合設定不會複製或覆蓋它。</small></div></div>${variableTokens(answerVariables)}${field("答案內容", "template_text", friendly, { wide: true, type: "textarea", rows: 12, placeholder: "您好，訂單{訂單編號}…" })}</section>
  ...
  function variableCard(v, i, q, vars) {
    const inputKind = v.multiline ? "textarea" : (v.type === "date" ? "date" : "text");
    const typeOptions = `<option value="text" ${inputKind === "text" ? "selected" : ""}>單行文字</option><option value="textarea" ${inputKind === "textarea" ? "selected" : ""}>多行文字</option><option value="date" ${inputKind === "date" ? "selected" : ""}>日期</option>`;
    return `<div class="subrecord-card"><div class="subrecord-head"><b>欄位 ${i + 1}｜${esc(v.label)}</b><button type="button" data-remove-variable="${i}">移除</button></div><input type="hidden" name="var_code_${i}" value="${esc(v.code)}"><div class="editor-grid">${field("中文欄位名稱", `var_label_${i}`, v.label, { required: true })}${field("輸入提示", `var_hint_${i}`, v.hint || "")}${field("值從哪裡找（顯示並附加於最終答案底部）", `var_source_note_${i}`, v.sourceNote || "", { wide: true, placeholder: "例如：訂單後台 → 訂單詳情" })}${field("來源連結（選填）", `var_source_url_${i}`, v.sourceUrl || "", { wide: true, type: "url", placeholder: "https://…" })}${field("輸入類型", `var_type_${i}`, inputKind, { type: "select", choices: typeOptions })}${field("自動依照哪個欄位", `var_source_${i}`, v.autoSource || "", { type: "select", choices: sourceChoices(q, vars, v.autoSource || "") })}${field("增減天數", `var_days_${i}`, v.autoDays ?? 0, { type: "number", hint: "15 為加 15 天；-1 為減 1 天" })}${field("必填", `var_required_${i}`, v.required, { type: "checkbox", checkLabel: "必填" })}${field("自動帶入", `var_common_${i}`, v.common, { type: "checkbox", checkLabel: "自動帶入上次使用的值" })}</div></div>`;
  }
  ...
  function handleFormClick(event) {
    ...
    const removeFieldAssignmentButton = event.target.closest("[data-remove-field-assignment]"); if (removeFieldAssignmentButton) { removeFieldAssignment(Number(removeFieldAssignmentButton.dataset.removeFieldAssignment), Number(removeFieldAssignmentButton.dataset.assignmentIndex)); return; }
    const editFieldButton = event.target.closest("[data-edit-field-code]");
    if (editFieldButton) {
      const index = data.fields.findIndex(fieldItem => fieldItem.code === editFieldButton.dataset.editFieldCode);
      if (index >= 0) { state.mode = "fields"; state.index = index; state.query = ""; $("#studioSearch").value = ""; renderStudio(); }
      return;
    }
  ...
  function addField() {
    data.fields.push({ code: nextId("V", [...data.fields, ...data.variables].map(v => v.code)), label: "新欄位", category: "未分類", hint: "", type: "text", required: true, fillRules: [] });
  ...
  function addBranch(questionName) { data.flows.push({ question: questionName || data.questions[0]?.name || "", branch: "新分支", steps: [], routes: [], answerParts: [{ question: questionName || data.questions[0]?.name || "", branch: "新分支" }], next: "" }); state.mode = "branches"; state.index = data.flows.length - 1; markDirty(); renderStudio(); }
  ...
  function saveField(show = true) {
    ...
    const inputKind = String(fd.get("field_type") || "text");
    ...
    Object.assign(item, {
      ...
      sourceNote: String(fd.get("field_source_note") || "").trim(), sourceUrl: String(fd.get("field_source_url") || "").trim(), type: inputKind === "date" ? "date" : "text",
  ...
  function captureBranchForm(flow, oldQ, oldBranch) {
    ...
    const previousVars = exactVariables(oldQ, oldBranch); const variables = previousVars.map((old, i) => ({ q, branch, code: String(fd.get(`var_code_${i}`) || old.code), label: String(fd.get(`var_label_${i}`) || "").trim(), hint: String(fd.get(`var_hint_${i}`) || "").trim(), sourceNote: String(fd.get(`var_source_note_${i}`) || "").trim(), sourceUrl: String(fd.get(`var_source_url_${i}`) || "").trim(), inputKind: String(fd.get(`var_type_${i}`) || "text"), autoSource: String(fd.get(`var_source_${i}`) || "") || undefined, autoDays: Number(fd.get(`var_days_${i}`) || 0), required: fd.has(`var_required_${i}`), common: fd.has(`var_common_${i}`) }));
    variables.forEach(v => {
      v.type = v.inputKind === "date" ? "date" : "text";
  ...
  function addVariable() {
    ...
    const item = { code: nextId("V", [...data.fields, ...data.variables].map(v => v.code)), label: "新欄位", category: "未分類", hint: "", type: "text", required: true, fillRules: [] };
  ...
  function removeAnswerPart(index) {
    if (!saveBranch(false)) return; const flow = current();
    flow.answerParts.splice(index, 1); markDirty(); renderForm();
  }
  function moveAnswerPart(index, direction) {
    if (!saveBranch(false)) return; const list = current().answerParts || []; const target = index + direction;
  ...
  function addFieldRule() {
    ...
    item.fillRules ||= []; item.fillRules.push({ values: [""], assignments: [] }); markDirty(); renderForm();
  }
  function removeFieldRule(ruleIndex) {
    if (!saveField(false)) return; current().fillRules.splice(ruleIndex, 1); markDirty(); renderForm();
  }
  function addFieldAssignment(ruleIndex) {
    if (!saveField(false)) return; const item = current();
