(() => {
  ...
  function inheritedVariablesFor(flow) {
    ...
    const previousParts = currentIndex >= 0 ? parts.slice(0, currentIndex) : parts;
  ...
  function branchForm(flow) {
    ...
    const answerCompositionSection = flow.question === "共用" ? "" : `<section class="editor-section wide"><div class="editor-section-title"><div><b>最終答案組合</b><small>依順序組合多個分支的獨立答案文字；同一分支可在不同流程重複使用。</small></div><button type="button" data-add-answer-part>＋ 加入答案分支</button></div><div class="branch-answer-list">${answerRows}</div><label class="studio-field wide composition-preview"><span>組合後內容預覽</span><textarea id="answerCompositionPreview" rows="12" readonly>${esc(compositionPreview(flow, answerParts, template?.text || ""))}</textarea><small>依目前順序預覽；欄位保留為 {中文欄位}，符合值後才出現的條件文字會在實際操作時加入。</small></label></section>`;
  ...
  function addField() {
    data.fields.push({ code: nextId("V", [...data.fields, ...data.variables].map(v => v.code)), label: "新欄位", category: "未分類", hint: "", type: "text", required: true, options: [], defaultValue: "", fillRules: [] });
  ...
  function addVariable() {
    ...
    const item = { code: nextId("V", [...data.fields, ...data.variables].map(v => v.code)), label: "新欄位", category: "未分類", hint: "", type: "text", required: true, options: [], defaultValue: "", fillRules: [] };
